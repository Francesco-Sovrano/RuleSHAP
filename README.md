...
Official code for the paper **"Can Global XAI Methods Reveal Injected Behaviours in LLMs? SHAP vs Rule Extraction vs RuleSHAP"**.

Large precomputed reproducibility artifacts are archived on Zenodo: [10.5281/zenodo.20561036](https://doi.org/10.5281/zenodo.20561036).

The project evaluates whether global explainability methods can detect deliberately injected misinformation-related behaviours in large language model outputs. It converts text prompts and model responses into ordinal abstractions, computes SHAP explanations, extracts global rules, and compares RuleSHAP against RuleFit, decision-tree baselines, GELPE, and SHAP-only feature rankings. Linear coefficients are exported as diagnostic baselines, but linear regression is not treated as the main rule-extraction baseline.
...
When the Zenodo `cache.zip` archive is extracted, many analyses can be rerun without repeating the original LLM calls. API keys are mainly required when regenerating missing model outputs or running new model configurations.
...
> **Note:** `run_all_experiments.sh` regenerates the full pipeline and may require API calls, local Llama models, substantial compute time, and disk space. If you only want to inspect or reproduce the reported analyses from the released artifacts, first download and unzip the three Zenodo archives above, then run only the relevant evaluation scripts.
...
- `correlation_analysis/`: input-output and LLM-as-a-judge correlation analyses. `correlation_analysis/` may be regenerated by `12_input_output_correlation_analysis.py` and `13_llm_estimate_proxy_metrics_correlation_analysis.py`.
